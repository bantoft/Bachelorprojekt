...
from loss_funktion.api.bout_struct import BoundaryCondition, HeselDerivedParameters
from loss_funktion.api.read_bout import *


class BOUTHESELInfo1:
    def __init__(self, hesel_path: Path, data_folder_name: str):
        self.root = Path(hesel_path).resolve()
        self.folder = data_folder_name

        self.settings = self._read_bout_settings()
        self.data = self._load_data(0)
    
    def _read_bout_settings(self) -> dict[str, Any]:
        settings_path = self.root / self.folder / 'BOUT.settings'
        section = 'root'
        settings: dict[str, Any] = {'root': {}}
        for raw in settings_path.read_text(encoding='utf-8').splitlines():
            ...
            if '=' in line:
                k, v = line.split('=', 1)
                settings[section][k.strip()] = v.strip()
        return settings
    
    def _load_data(self) -> dict[str, Any]:
        data_path = self.root / self.folder / f'BOUT.dmp.{num}.nc'
...
hesel_path = Path(__file__).resolve().parents[1] / r'simulatorer/BOUT/BOUT-HESEL'
data_folder = "data"

info1 = BOUTHESELInfo1(hesel_path, data_folder)

# for key, value in info1.data.items():
#     print(key, value.shape)

# print(info1.settings)