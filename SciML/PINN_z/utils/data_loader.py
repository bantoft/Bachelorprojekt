...
class HESEL_z_Dataset(Dataset):
    def __init__(self, info, m: int = 3):
        ...
        self.z_half = self.m // 2
        self.z_offsets = torch.arange(-self.z_half, self.z_half + 1, dtype=torch.long)
    ...
    def __getitem__(self, idx):
        ...
        center_shape = (1, self.nx, 1)


        u = torch.cat([self._window(t_idx, z_idx), self._window(t_idx + 1, z_idx)], dim=0)
        y = self._window(t_idx + 2, z_idx)[:, :, self.z_half : self.z_half + 1]
        
        avg_z = self.avg_z[t_idx + 2].unsqueeze(-1)

        # Kun center for z og t, wrapper expander selv ud
        coords_fys = torch.tensor([self.dz * z_idx, self.dt * (t_idx + 2)], dtype=self.dtype)

        x_idx_expanded = self.x_idx.expand(*center_shape)
        z_idx_expanded = torch.full(center_shape, z_idx, dtype=torch.long)
        t_idx_tensor = torch.full(center_shape, t_idx + 2, dtype=torch.long)
