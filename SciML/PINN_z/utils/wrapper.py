...
class WrappedPINN(torch.nn.Module):
    def __init__(self, network_structure, info, phys, m: int = 3):
        ...
        self.num_output_channels = len(FIELDS)
        ...
        self.line_output_size = self.num_output_channels * self.nx
        self.pinn = PINN(self._resolve_network_structure(network_structure)).to(self.device)
    ...
    def _resolve_network_structure(self, network_structure):
        resolved_structure = dict(network_structure)
        resolved_structure["input_size"] = self.line_input_size
        resolved_structure["output_size"] = self.line_output_size
        resolved_structure["output_names"] = tuple(network_structure["output_names"])
    ...
    def forward(self, batch, requires_coord_grad: bool = True):
        ...
        D_x = self.D_x[:, :, self.z_half : self.z_half + 1]
